...
void SociAuthDB::getUserWithPhoneWithPool(const std::string &phone, AuthDbListener *listener) {
	steady_clock::time_point start;
	steady_clock::time_point stop;
	std::string user;
	session *sql = NULL;
...
void SociAuthDB::getUserWithPhoneFromBackend(const char *phone, AuthDbListener *listener) {

	// create a thread to grab a pool connection and use it to retrieve the auth information
	auto func = bind(&SociAuthDB::getUserWithPhoneWithPool, this, std::string(phone), listener);
